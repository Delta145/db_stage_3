...
if __name__ == '__main__':
    ...
    START_WORK = 8
    END_WORK = 20
    DEFAULT_FILE = 'insert_script.sql'
    ...
    with open("data_adr.csv", 'r', encoding='utf-8') as f:
        for s in f.readlines():
            ...
            insert_into(table_name, fields, values, 'insert_addresses.sql')
    ...
    def person_generator(num):
        for i in range(10000000):
            name = names[choice(range(1, len(names)))]
            surname = surnames[choice(range(1, len(surnames)))]
            sex = sexs[choice(range(1, len(sexs)))]
            yield {
                'имя': f"'{name}'",
                'фамилия': f"'{surname}'",
                'пол': f"'{sex}'",
    ...
    for s in person_generator(PERSON_AMOUNT):
        ...
        insert_into(table_name, fields, values, 'insert_persons.sql')
    ...
    with open("adjectives.csv", 'r', encoding='utf-8') as f:
        ...
        for s in f.readlines():
            ...
            insert_into(table_name, fields, values, 'insert_coffees.sql')
    ...
    for order_number in range(1, STD_AMOUNT):
        ...
        insert_into(table_name, fields, values, 'insert_orders.sql')
        table_name = "компонент_заказа"
        fields = ['id_заказа', 'id_товара']
        for i in component:
            insert_into(table_name, fields, i, 'insert_order_components.sql')
    ...
    with open("ingredients.csv", 'r', encoding='utf-8') as f:
        ...
        for s in f.readlines():
            arr = s.replace("\n", "").split(";")
            values = arr[:3]
            insert_into(table_name, fields, values, 'insert_ingredients.sql')
    ...
    for i in range(STD_AMOUNT):
        ...
        insert_into(table_name, fields, values, 'insert_schedules.sql')
        for j in range(7):
            ...
            insert_into(table_name, fields, values, 'insert_schedule_components.sql')
    ...
    for i in range(STD_AMOUNT):
        ...
        values = [
            choice(range(1, PERSON_AMOUNT)),
            choice(range(14, COFFEE_AMOUNT + 13))
        ]
        insert_into(table_name, fields, values, 'insert_favorite_coffees.sql')
    ...
    for i in range(STD_AMOUNT):
        ...
        insert_into(table_name, fields, values, 'insert_favorite_schedules.sql')
    ...
    with open("adjectives.csv", 'r', encoding='utf-8') as f:
        for i in range(50):
            ...
            insert_into(table_name, fields, values, 'insert_coffee_scores.sql')
        ...
        for i in range(50, 100):
            ...
            insert_into(table_name, fields, values, 'insert_schedule_scores.sql')
